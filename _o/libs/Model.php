...
class Model {
	...
	function __construct($model = false) 
	{
		...
		if (in_array($model, $this->db->tables) || $this->table_sql)
		{
			...
			{
				if (!$this->table_schema = $this->db->fields($this->table))
				{
	...
	function exists($id)
    {
		...
		return $this->find_row(array('id' => $id));
    }
    
    
    function find($where = array(), $args = array())
    {
		$defaults = array(
			'page' => 0,
			'num' => '*',
		...
		$query = "SELECT ".join(',', $fields)." FROM `$this->table` WHERE 1 = 1 AND ".$this->where('AND', $where);
	...
	function find_row($where)
	{
		return $this->find($where, array('num' => 1));
	}
	
	
	function find_item($field, $where)
	{
		return $this->find($where, array('num' => 1, 'fields' => array($field)));
	}
    ...
    function edit($params)
    {
        ...
        if ($this->db->update($this->table, $params, compact('id')))
            return $id;
    ...
    protected function validate(&$params)
    {
    	foreach ($params as $key => $value) :
			...
			if (!isset($params['id']) && in_array($this->table_schema[$key]['Key'], array('PRI', 'UNI')))
				if ($this->find_row(array($key => $value)))
